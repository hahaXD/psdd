...
namespace {
SddNode *CardinalityK(
    uint32_t variable_size_usign, uint32_t k, SddManager *manager,
    std::unordered_map<uint32_t, std::unordered_map<uint32_t, SddNode *>>
        *cache) {
  ...
  if (variable_size == 1) {
    ...
    if (k == 0) {
      SddNode *remaining =
          CardinalityK(variable_size_usign - 1, 0, manager, cache);
      SddNode *result = sdd_conjoin(
          sdd_manager_literal(-variable_size, manager), remaining, manager);
      cache_it->second.insert({0, result});
      return result;
    } else if (k == variable_size) {
      SddNode *remaining =
          CardinalityK(variable_size_usign - 1, k - 1, manager, cache);
      SddNode *result = sdd_conjoin(sdd_manager_literal(variable_size, manager),
                                    remaining, manager);
      cache_it->second.insert({k, result});
      return result;
    } else {
      SddNode *remaining_positive =
          CardinalityK(variable_size_usign - 1, k - 1, manager, cache);
      SddNode *remaining_negative =
          CardinalityK(variable_size_usign - 1, k, manager, cache);
      SddNode *positive_case =
          sdd_conjoin(sdd_manager_literal(variable_size, manager),
                      remaining_positive, manager);
      SddNode *negative_case =
          sdd_conjoin(sdd_manager_literal(-variable_size, manager),
                      remaining_negative, manager);
...
TEST(SDD_TO_PSDD_TEST, VARIABLE_MAP_TEST) {
  ...
  auto serialized_psdd = psdd_node_util::SerializePsddNodes(result_psdd);
  {
    ...
    EXPECT_TRUE(!psdd_node_util::IsConsistent(serialized_psdd, variable_mask,
...
TEST(PSDD_MANAGER_TEST, GET_TOP_NODE) {
  ...
  PsddTopNode *new_top_node =
      psdd_manager->GetPsddTopNode(1, 0, PsddParameter::CreateFromDecimal(0.1),
                                   PsddParameter::CreateFromDecimal(0.9));
  EXPECT_EQ(new_top_node->true_parameter(),
            PsddParameter::CreateFromDecimal(0.1));
  EXPECT_EQ(new_top_node->false_parameter(),
            PsddParameter::CreateFromDecimal(0.9));
  PsddTopNode *second_new_top_node =
      psdd_manager->GetPsddTopNode(1, 0, PsddParameter::CreateFromDecimal(0.1),
                                   PsddParameter::CreateFromDecimal(0.9));
  EXPECT_EQ(second_new_top_node, new_top_node);
  second_new_top_node =
      psdd_manager->GetPsddTopNode(1, 0, PsddParameter::CreateFromDecimal(0.9),
                                   PsddParameter::CreateFromDecimal(0.1));
  EXPECT_NE(second_new_top_node, new_top_node);
  EXPECT_EQ(second_new_top_node->true_parameter(),
            PsddParameter::CreateFromDecimal(0.9));
  EXPECT_EQ(second_new_top_node->false_parameter(),
            PsddParameter::CreateFromDecimal(0.1));
  second_new_top_node =
      psdd_manager->GetPsddTopNode(1, 1, PsddParameter::CreateFromDecimal(0.1),
                                   PsddParameter::CreateFromDecimal(0.9));
  EXPECT_NE(second_new_top_node, new_top_node);
  delete (psdd_manager);
}
...
TEST(PSDD_MANAGER_TEST, GET_LIERAL_NODE) {
  ...
  PsddLiteralNode *second_new_literal_node =
      psdd_manager->GetPsddLiteralNode(-1, 0);
...
TEST(PSDD_MANAGER_TEST, GET_CONFORMED_DECISION_NODE) {
  ...
  PsddTopNode *top_ten =
      psdd_manager->GetPsddTopNode(10, 0, PsddParameter::CreateFromDecimal(0.9),
                                   PsddParameter::CreateFromDecimal(0.1));
  PsddLiteralNode *pos_one = psdd_manager->GetPsddLiteralNode(1, 0);
  PsddTopNode *top_ten_second =
      psdd_manager->GetPsddTopNode(10, 0, PsddParameter::CreateFromDecimal(0.1),
                                   PsddParameter::CreateFromDecimal(0.9));
  PsddDecisionNode *new_decn_node = psdd_manager->GetConformedPsddDecisionNode(
      {pos_one, neg_one}, {top_ten, top_ten_second},
      {PsddParameter::CreateFromDecimal(0.8),
       PsddParameter::CreateFromDecimal(0.2)},
      0);
  PsddDecisionNode *new_second_node =
      psdd_manager->GetConformedPsddDecisionNode(
          {neg_one, pos_one}, {top_ten_second, top_ten},
          {PsddParameter::CreateFromDecimal(0.2),
           PsddParameter::CreateFromDecimal(0.8)},
          0);
  ...
  Probability pr =
      psdd_node_util::Evaluate(variables, instantiation_one, new_decn_node);
  Probability pr2 =
      psdd_node_util::Evaluate(variables, instantiation_two, new_decn_node);
  EXPECT_EQ(pr, pr2);
...
TEST(PSDD_MANAGER_TEST, FROM_SDD_TEST) {
  Vtree *v = sdd_vtree_new(8, "balanced");
  SddManager *manager = sdd_manager_new(v);
  sdd_vtree_free(v);
  std::unordered_map<uint32_t, std::unordered_map<uint32_t, SddNode *>> cache;
  SddNode *card_node = CardinalityK(8, 4, manager, &cache);
  Vtree *psdd_vtree = sdd_vtree_new(16, "balanced");
  PsddManager *psdd_manager = PsddManager::GetPsddManagerFromVtree(psdd_vtree);
  sdd_vtree_free(psdd_vtree);
  std::unordered_set<SddLiteral> used_variables;
  for (SddLiteral i = 1; i <= 8; ++i) {
    used_variables.insert(i);
  }
  PsddNode *node1 = psdd_manager->FromSdd(card_node, sdd_manager_vtree(manager),
                                          0, used_variables);
  auto sdd_mc = sdd_model_count(card_node, manager);
  auto node_1_mc =
      psdd_node_util::ModelCount(psdd_node_util::SerializePsddNodes(node1));
  EXPECT_EQ(std::to_string(sdd_mc), node_1_mc.get_str(10));
  used_variables.clear();
  for (SddLiteral i = 1; i <= 8; ++i) {
    used_variables.insert(i + 8);
  }
  PsddNode *node2 = psdd_manager->FromSdd(card_node, sdd_manager_vtree(manager),
                                          0, used_variables);
  auto node_2_mc =
      psdd_node_util::ModelCount(psdd_node_util::SerializePsddNodes(node2));
  EXPECT_EQ(std::to_string(sdd_mc), node_2_mc.get_str(10));
  std::unordered_map<uint32_t, uint32_t> variable_identical_map;
  PsddNode *node3 =
      psdd_manager->NormalizePsddNode(psdd_manager->vtree(), node1, 0);
  auto node_3_mc =
      psdd_node_util::ModelCount(psdd_node_util::SerializePsddNodes(node3));
  EXPECT_EQ(std::to_string(sdd_mc << 8), node_3_mc.get_str(10));
  delete (psdd_manager);
...
TEST(PSDD_MANAGER_TEST, LOAD_PSDD_TEST) {
  ...
  PsddManager *test_psdd_manager =
      PsddManager::GetPsddManagerFromSddVtree(v, variable_identical_map);
  sdd_vtree_free(v);
  std::unordered_map<uint32_t, std::unordered_map<uint32_t, SddNode *>> cache;
  ...
  second_card_psdd =
      test_psdd_manager->LoadPsddNode(test_psdd_manager->vtree(), card_psdd, 1);
  EXPECT_NE(second_card_psdd, card_psdd);
  size_t bound = 1 << 9;
  std::bitset<MAX_VAR> variable_mask = bound - 1;
  std::vector<PsddNode *> serialized_card_psdd =
  ...
  for (size_t i = 0; i < bound; ++i) {
    ...
    EXPECT_EQ(psdd_node_util::IsConsistent(serialized_card_psdd, variable_mask,
                                           cur_instantiation),
              psdd_node_util::IsConsistent(serialized_second_card_psdd,
                                           variable_mask, cur_instantiation));
  }
  ...
  PsddManager *different_psdd_manager =
      PsddManager::GetPsddManagerFromSddVtree(second_v, variable_map);
  PsddNode *second_psdd = different_psdd_manager->LoadPsddNode(
      different_psdd_manager->vtree(), card_psdd, 0);
  sdd_vtree_free(second_v);
  std::vector<PsddNode *> serialized_second_psdd =
      psdd_node_util::SerializePsddNodes(second_psdd);
  for (auto i = 1; i <= 16; ++i) {
    std::bitset<MAX_VAR> cur_instantiation = i;
    EXPECT_EQ(psdd_node_util::IsConsistent(serialized_card_psdd, variable_mask,
                                           cur_instantiation),
              psdd_node_util::IsConsistent(serialized_second_psdd,
                                           variable_mask, cur_instantiation));
  }
  auto second_node_mc =
      psdd_node_util::ModelCount(serialized_second_psdd).get_ui();
...
TEST(PSDD_MANAGER_TEST, MULTIPLY_TEST) {
  ...
  PsddNode *one_true = manager->NormalizePsddNode(
      manager->vtree(), manager->GetPsddLiteralNode(1, 0), 0);
  PsddNode *one_false = manager->NormalizePsddNode(
      manager->vtree(), manager->GetPsddLiteralNode(-1, 0), 0);
...
TEST(PSDD_MANAGER_TEST, MULTIPLY_TEST2) {
  ...
  for (auto i = 0; i < cap; ++i) {
    std::bitset<MAX_VAR> cur_instantiation = i;
    PsddParameter cur_num =
        psdd_node_util::Evaluate(mask, cur_instantiation, serialized_node_1) *
        psdd_node_util::Evaluate(mask, cur_instantiation, serialized_node_2);
    PsddParameter result_num =
        psdd_node_util::Evaluate(mask, cur_instantiation, result_s_psdd) *
        result.second;
...
TEST(PSDD_MANAGER_TEST, MULTIPLY_TEST3) {
  ...
  for (auto i = 0; i < cap; ++i) {
    std::bitset<MAX_VAR> cur_instantiation = i;
    PsddParameter cur_num =
        psdd_node_util::Evaluate(mask, cur_instantiation, s_node_1) *
