...
class PsddNode {
 public:
  ...
  virtual int node_type() const = 0; // 1 is literal node, 2 is decision node, 3 is top node with variable index
  ...
  virtual bool IsConsistent(const std::unordered_map<uint32_t, bool> &partial_instantiation) const = 0;
  bool IsConsistent(const std::bitset<MAX_VAR> &instantiation, uint32_t variable_size);
  std::size_t hash_value() const;
  Vtree *vtree_node() const;
  virtual void CalculateParametersUsingLaplacianSmoothing(PsddParameter alpha);
...
class PsddLiteralNode : public PsddNode {
 public:
  PsddLiteralNode(uintmax_t node_index, Vtree *vtree_node, uintmax_t flag_index, int32_t literal);
  PsddLiteralNode(uintmax_t *node_index, Vtree *vtree_node, uintmax_t flag_index, int32_t literal);
  ...
  bool IsConsistent(const std::unordered_map<uint32_t, bool> &partial_instantiation) const override;
  bool sign() const;
...
class PsddDecisionNode : public PsddNode {
 public:
  PsddDecisionNode(uintmax_t node_index,
                   Vtree *vtree_node,
...
class PsddTopNode : public PsddNode {
 public:
  PsddTopNode(uintmax_t node_index,
              Vtree *vtree_node,
              uintmax_t flag_index,
              uint32_t variable_index,
              PsddParameter true_parameter,
              PsddParameter false_parameter);
  PsddTopNode(uintmax_t *node_index, Vtree *vtree_node, uintmax_t flag_index, uint32_t variable_index);
  PsddTopNode(uintmax_t *node_index, Vtree *vtree_node, uint32_t variable_index);
