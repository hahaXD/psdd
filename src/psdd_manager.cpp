...
class PsddManagerImp : public PsddManager {
 public:
  explicit PsddManagerImp(Vtree *sdd_vtree, const std::unordered_map<uint32_t, uint32_t> &variable_mapping)
      : node_index_(0) {
    ...
    vtree_ = (Vtree *) sdd_vtree_data(sdd_vtree);
    set_vtree_properties(vtree_);
    for (Vtree *sdd_vtree_node : serialized_sdd_vtree) {
      sdd_vtree_set_data(nullptr, sdd_vtree_node);
    }
    unique_table_ = PsddUniqueTable::GetPsddUniqueTable();
  }
  ~PsddManagerImp() override {
    unique_table_->DeleteUnusedPsddNodes({});
    delete (unique_table_);
    sdd_vtree_free(vtree_);
  }
  void DeleteUnusedPsddNodes(const std::vector<PsddNode *> &used_nodes) override {
    unique_table_->DeleteUnusedPsddNodes(used_nodes);
  }
  PsddNode *ConvertSddToPsdd(SddNode *root_node,
                             Vtree *sdd_vtree,
                             uintmax_t flag_index,
                             const std::unordered_map<uint32_t, uint32_t> &variable_mapping) override {
...
PsddManager *PsddManager::GetPsddManager(Vtree *sdd_vtree,
  ...
  return new PsddManagerImp(sdd_vtree, variable_mapping);
}
